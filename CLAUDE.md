...
- `@unai/api`: `createApiBoundary` (`index.ts`) is the generic Fastify boundary; `createPlatformApi` (`platform.ts`) is the production composition with device, evidence (`evidence.ts`), governed memory write (`memory.ts`), correction control (`corrections.ts`), typed projection read (`projections.ts`), Context Broker and inspection (`context.ts`) and ops (`ops.ts`) routes; `server.ts` is the entry point.
