...
- `@unai/api`: `createApiBoundary` (`index.ts`) is the generic Fastify boundary; `createPlatformApi` (`platform.ts`) is the production composition with device, evidence (`evidence.ts`), governed memory write (`memory.ts`), correction control (`corrections.ts`), typed projection read (`projections.ts`), Context Broker and inspection (`context.ts`), connector, sync and document (`connectors.ts`), question answering (`ask.ts`, `POST /v1/ask` under `memory.read`), answer provenance (`answers.ts`, the answer recorder and `GET /v1/answers/{id}/manifest` and `/v1/answers/reconsideration-candidates` under `memory.inspect`) and ops (`ops.ts`) routes; `server.ts` is the entry point.
