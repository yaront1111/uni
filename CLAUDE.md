...
Adding a route: add its purpose string to `PLATFORM_PURPOSES` **and** the URL-to-purpose chain `routePurpose` in `platform.ts` (the security suite's object-key sweep in `transport.test.ts` discovers every `app.get('/v1/…')` from source and declares `routePurpose` for it), to the web proxy mapping if the browser calls it, and to the SQL policy when the table gates on purpose (as `jobs` does).
...
### Security and accessibility suites

The CI security suite is `packages/api/src/security.test.ts` (one test per CRT-SEC-10-A case, including the malicious email/document and secret-redaction scenarios with deliberately *obeying* model doubles), `transport.test.ts` (TLS, encryption at rest, no raw object key in any GET response) and `audit.test.ts`; they need the full harness, including real object storage. `apps/web/components/Accessibility.test.ts` runs axe-core in a jsdom document (`// @vitest-environment jsdom`) and a keyboard-only user (`components/testing/keyboard.ts`) over the core views, and writes `test-results/accessibility/` (gitignored, uploaded by CI). It needs no database: `pnpm exec vitest run apps/web/components/Accessibility.test.ts`. Report: `docs/accessibility-audit-and-security.md`.
