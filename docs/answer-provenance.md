...
CRT-RYW-05-A. ADR 0024 records its decisions; ADRs 0001–0023 and every
...
`migrations/0019_answer_manifests_and_reconsideration.sql` with forced RLS,
...
- Tests of earlier nodes adapted to the new schema, never weakened: the Ask route
  ...
  projection replay drops and expects migration 0019.
...
- **Personal facts are checked by object and by number.** Free text naming an
  entity is not matched against the packet (ADR 0024 §3).
