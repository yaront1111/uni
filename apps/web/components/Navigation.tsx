...
const navigation=['Commitments','Decisions','Weekly Review','Memory Inspector','Memory Inbox','Permissions'];
/** `current` is null on a screen reached from another one rather than from the
 * navigation, such as an answer's provenance. */
export function Navigation({current}:{current:'today'|'ask'|'devices'|'sources'|'connectors'|'jobs'|'registry'|null}){
