...
export default async function handler(req:NextApiRequest,res:NextApiResponse){
  ...
  try{
    ...
    const response=await apiRequest('/v1/'+path,'POST',{cookie:req.headers.cookie??'','x-owner-scope-id':session.ownerScopeId,'x-purpose':purpose,'x-correlation-id':correlation,'idempotency-key':key,
      // Evidence ingest, and a merge or split (whose governed commit reads the
      // evidence behind the claims it reassigns), pass the evidence gate pinned.
      ...(path==='evidence'||purpose==='memory.govern'?{'x-data-purpose':'PERSONAL_ASSISTANCE','x-maximum-sensitivity':'RESTRICTED'}:{})},body);
