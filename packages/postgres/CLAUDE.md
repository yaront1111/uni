...
3. Bump the table-count literal in "forces RLS on all application tables" (`26` after migration 0010); it counts every `public` table, global ones included. After migration 0022 it is 58: 55 owner-scoped tables and the three global ones (`registry_releases`, `registry_contracts`, `registry_migration_manifests`).
...
5. `packages/capabilities/src/projection-replay.test.ts` drops everything migrations 0016 onward created and re-applies them: add the new migration's tables and functions to its drop list and the file to its expected applied list.
