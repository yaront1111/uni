...
3. Bump the table-count literal in "forces RLS on all application tables" (`26` after migration 0010); it counts every `public` table, global ones included. After migration 0025 it is 74: 71 owner-scoped tables and the three global ones (`registry_releases`, `registry_contracts`, `registry_migration_manifests`).
