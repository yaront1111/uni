...
This folder owns the `pnpm test` harness (`test.mjs` plus its storage and TLS helpers) and the manual `verifier-rehearsal.mjs` diagnostic. It must not own product code or test cases: nothing under `packages/`, `apps/` or `src/` imports from here, and the only entry points are the `test` script in the root `package.json` and `node scripts/verifier-rehearsal.mjs`.
